...
def generate_optimized_bullets(missing_keywords: List[str]) -> List[str]:
    ...
    if missing_keywords:
        ...
        bullets.append(f"Optimized core operational processes utilizing {kw1} and {kw2} to enhance overall project performance.")
    ...
    if len(missing_keywords) > 4:
        ...
        bullets.append(f"Spearheaded cross-functional initiatives leveraging {kw5} to maximize efficiency and achieve target deliverables.")
    if not bullets:
...
<head>
    ...
    <title>DOMA AI - Resume Matcher & Rewriter</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
</head>
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col font-sans">
    <nav class="border-b border-slate-800 bg-slate-900/90 px-6 py-4 flex justify-between items-center sticky top-0 z-40">
        ...
        <div id="auth-nav" class="flex items-center space-x-4">
            ...
            <button onclick="openModal('login-modal')" id="login-btn" class="text-sm font-semibold hover:text-blue-400 transition px-3 py-2">Log In</button>
            ...
            <button onclick="logout()" id="logout-btn" class="hidden text-sm text-red-400 hover:text-red-300 font-semibold transition px-3 py-2">Log Out</button>
        </div>
    </nav>

    <main class="flex-1 max-w-7xl w-full mx-auto p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between space-y-6">
            <div>
                ...
                <textarea id="jd_text" class="w-full h-44 bg-slate-950 border border-slate-800 rounded-xl p-4 text-sm text-slate-200 focus:outline-none focus:border-blue-500 transition resize-none" placeholder="Paste target job responsibilities and key requirements here..."></textarea>
            </div>
            ...
            <div>
                ...
                <label class="border-2 border-dashed border-slate-800 hover:border-blue-500 bg-slate-950/60 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition">
            ...
            <button onclick="runAnalysis()" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-xl shadow-lg transition flex items-center justify-center space-x-2">
                <i class="fa-solid fa-bolt"></i>
                <span>Match & Auto-Rewrite Resume</span>
            </button>
        </div>

        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 flex flex-col space-y-6">
            ...
            <div id="results-placeholder" class="flex-1 flex flex-col items-center justify-center text-center p-8 border border-dashed border-slate-800 rounded-xl">
                <i class="fa-solid fa-wand-magic-sparkles text-4xl text-slate-700 mb-3"></i>
            ...
            <div id="results-content" class="hidden flex-1 flex-col space-y-6">
                <div class="flex items-center space-x-6 bg-slate-950 p-4 rounded-xl border border-slate-800">
                ...
                <div class="space-y-3 flex-1 flex flex-col">
                    ...
                    <div id="bullets-list" class="bg-slate-950 p-4 rounded-xl border border-slate-800 text-sm space-y-2 text-slate-300 flex-1 overflow-y-auto max-h-48"></div>
                </div>
    ...
    <!-- Modal Containers -->
    <div id="login-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center p-4 z-50">
        <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-sm w-full space-y-4">
            ...
            <input id="login-email" type="email" placeholder="Email Address" class="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-blue-500">
            <input id="login-pass" type="password" placeholder="Password" class="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-blue-500">
    ...
    <div id="signup-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center p-4 z-50">
        <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-sm w-full space-y-4">
            <div class="flex justify-between items-center">
            ...
            <input id="signup-email" type="email" placeholder="Email Address" class="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-blue-500">
            <input id="signup-pass" type="password" placeholder="Password" class="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-blue-500">
    ...
    <script>
        ...
        async function handleSignup() {
            ...
            try {
                var res = await fetch('/auth/signup', { method: 'POST', body: body });
                if(res.ok) {
        ...
        async function handleLogin() {
            ...
            var body = new URLSearchParams({ username: email, password: pass });
            try {
                var res = await fetch('/auth/login', { method: 'POST', body: body });
                if(res.ok) {
                    var data = await res.json();
                    localStorage.setItem('token', data.access_token);
                    localStorage.setItem('coins', data.coins);
        ...
        async function runAnalysis() {
            var token = localStorage.getItem('token');
            if(!token) {
                alert('Please sign up or log in first to use coins!');
                openModal('signup-modal');
                return;
            }

            var jd = document.getElementById('jd_text').value;
            ...
            try {
                ...
                if(res.ok) {
                    ...
                    var tags = document.getElementById('missing-tags');
                    tags.innerHTML = '';
                    data.missing.forEach(function(kw) {
                        tags.innerHTML += '<span class="bg-red-500/20 text-red-300 text-xs px-2.5 py-1 rounded-full border border-red-500/30">' + kw + '</span>';
                    });

                    var bullets = document.getElementById('bullets-list');
                    bullets.innerHTML = '';
                    ...
                    alert('You ran out of coins! Upgrade your plan to continue.');
                } else {
